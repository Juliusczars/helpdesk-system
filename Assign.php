...
$date=$_POST['dateA'];
$officer=$_POST['Oassign'];
$timestamp=date('Y-m-d G:i:s');

$sql="INSERT into s_task values('$tno', NUll, '$date', '$officer', 'pending', NUll, '$login_session')";
...
if(empty($date)){$message="Failed.Please select a date!!";}
  ...
  if($result){
   // mysqli_query($db, $query);
    ...
    $message="Ticket assignment failed! Please check ur ticket number";
  }
   ...
   <div id="head-status" ><P style="">ICT HELPDESK SYSTEM</P></div>
	...
	<div id="bod">
		...
		<td><label for="PNO">
			...
			<label for="PENO">
				<span>Ext. No.</span>
				<input type="text" id="PENO" name="ENO" required>
			</label></td></tr>
			...
			<label for="fault">
				<span>Fault</span>
				<select id="fault" name="fault"style="margin:15px 0 15px 15px;" required>
				<option></option>
...
<div id="Proc-right" style="padding-top:60px;">
        ...
        <label for="Tno">
			...
			<label for="date">
				<span>Date</span>
				<input type="text" name="dateA" readonly="true" id="dat" required>
			</label>
			<label for="Oassign">
				<span>Officer Assigned</span>
				<input type="text" id="Oassign" name="Oassign" required>
			</label>
...
<div id="Proc-right" >
      ...
      <label for="date">
        ...
        <input type="text" name="date" id="datet" readonly="true" required>
      </label>
