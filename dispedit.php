...
$query="UPDATE p_dispatch SET Description='$desc', Serialno='$serial', Ddate='$date', Location='$loc', Department='$dept' WHERE SN='$Id'";
$result=mysqli_query($db, $query) ;
if($result){
echo "<script> alert('Dipatch Details updated successfully! ');</script>";

}
else{echo "<script> alert('Dipatch Details update failed! Confirm the Serial number.');</script>";}
...
<body>
	...
	<div id="sidebar-collapse" class="col-sm-3 col-lg-2 sidebar">
		...
		<ul class="nav menu">
			<li class="active"><a href="displist.php"><svg class="glyph stroked dashboard-dial"><use xlink:href="#stroked-dashboard-dial"></use></svg> Dispatch list</a></li>
			
			
			<li role="presentation" class="divider"></li>
			<li><a href="dispatch.php"><svg class="glyph stroked home"></svg> Back</a></li>
		</ul>
    ...
    <div class="modal fade" id="edit" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">Edit Dispatch Details</h4>
                </div>
               <form method="post" action="dispedit.php">
                <div class="modal-body">
                    <div class="row">
                         <input hidden id="user_id" name="user_id">
                        <div class="form-group col-xs-8">
                        	<label >Serial NO.</label>
                        	<input type="text" id="username" name="serial" class="form-control">  
                       </div>
                    </div>
                    <div class="row">
                        ...
                        <div class="form-group col-xs-8">
                            ...
                            <Select type="text" id="pid" name="pid" required class="form-control">
               ...
               <?php  
                 ...
                 while($row=mysqli_fetch_array($result)){
                     ...
                     echo "<tr>
                            ...
                            <td>
                                <div class='btn-group'>
                                    <button id='$t->SN' class='btn-default' data-toggle='modal' data-target='#edit'>Edit</button>
                                  <!--<button id='$t->SN' data-toggle='modal' data-target='#delete' class='btn-danger' >Delete</button> -->
                                </div> 
                            </td>
                        </tr>";
            ...
            var $row=$(this).closest('tr');
             var serial=$row.find('.serial').text();
               var desc=$row.find('.desc').text();
                var dept=$row.find('.dept').text();
                var date=$row.find('.date').text();
                 var loc=$row.find('.loc').text();
