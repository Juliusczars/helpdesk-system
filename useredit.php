...
$approv=mysql_real_escape_string($_POST['username']);
$btn=$_POST['user_id'];
$que="UPDATE users SET username='$approv' WHERE P_no='$btn'";



//if(ctype_alnum($pas)&& strlen($pas)>6 && preg_match('`[A-Z]`',$pas) && preg_match('`[a-z]`',$pas) && preg_match('`[0-9]`',$pas)){
	$res=mysqli_query($db, $que) or die(mysqli_error($db));
if($res){
 echo "<script> alert('User details updated successfully!');</script>";

}
else{ echo "<script> alert('User details update failed!');</script>";}
 //}
 //else{echo "<script> alert('Password not valid, must be atleast 6 characters contain atleast one lower and uppercase letter and a digit  ');</script>";}
?>
    ...
    <div class="modal fade" id="edit" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        ...
        <div  class="table-responsive col-lg-12" >
                ...
                <div class="ibox-content">
                    ...
                    <tbody>
               <?php  
                  $query="SELECT users.P_no, Fname, Lname, username, employee.Email from users, employee WHERE users.P_no=employee.P_no ";
                 ...
                 while($row=mysqli_fetch_array($result)){
                     ...
                     echo " <tr>
                            ...
                            <td>".$row['Fname']."</td>
                            <td>".$row['Lname']."</td>
                            <td class='us'>".$row['username']."</td>
                            <td>".$row['Email']."</td>
                            
                            
                            <td>
                                <div class='btn-group'>
                                    <button id='$t->P_no' class='btn-default' data-toggle='modal' data-target='#edit'>Edit</button>
                                  <button id='$t->P_no' data-toggle='modal' data-target='#delete' class='btn-danger' >Delete</button> 
                                </div> 
                            </td>
                        </tr>";
